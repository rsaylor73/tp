...
require_once('PKPass.php');
	...
	$qr = encrypt_decrypt('decrypt',$_GET['q']);

	// Create pass

	//Set certifivate and path in the constructor
	$pass = new PKPass($path . '/pass/certificates/tppass.p12', 'rb102573');

	// Add the WWDR certificate 
	$pass->setWWDRcertPath($path . '/pass/certificates/wwdr.pem');
	...
	"description": "Ticket Pointe :: '.$_GET['e'].'",
  "eventTicket" : {
    "primaryFields" : [
      {
        "key" : "event",
    ...
    "auxiliaryFields" : [
      {
        "key" : "date",
        "label" : "DATE",
        "value" : "'.$_GET['d1'].' to '.$_GET['d2'].'"
      },
      {
        "key" : "time",
        "label" : "TIME",
        "value" : "'.$_GET['t1']. ' to '.$_GET['t2'].'",

...
class Tickets {
	...
	public function signoff() {
        ...
        public function login($msg) {
				...
				<tr><td><input type=\"text\" name=\"uuname\" size=20></td></tr>
                                <tr><td><b>Password:</b></td></tr>
				<tr><td><input type=\"password\" name=\"uupass\" onkeypress=\"if(event.keyCode==13) { login(this.form); return false;}\" size=20></td></tr>
                                <tr><td><input type=\"button\" class=\"btn btn-warning\" value=\"Forgot Password?\">&nbsp;&nbsp;<input type=\"button\" class=\"btn btn-primary\" value=\"Login\" onclick=\"login(this.form)\"></td></tr>
                                ...
                                function login(myform) {
                                        ...
                                        function(php_msg) {
                                          if (php_msg.substring(0,4) == "http") {
                                             $("#login").html('<span class="details-description"><font color=green>Login successful. Loading please wait...</font><br></span>');
                                             setTimeout(function()
                                                {
	...
	public function upcoming_events() {
		...
		while ($row = $result->fetch_assoc()) {
				...
				$link2 = "</a>";
                                ...
                                $link = "<a href=\"index.php?section=page_view&id=$row[id]\"><font color=#096FBB>$row[title]</font></a>";
                                $link1 = "<a href=\"index.php?section=page_view&id=$row[id]\">";
				$link2 = "</a>";
                        }
			$views = $this->get_views($row['id']);
			$cover = "";
			if ($row['cover_image'] != "") {
				$cover = "<tr><td colspan=2><center>$link1<img src=\"uploads/$row[userID]/cover/$row[id]/$row[cover_image]\" width=\"300\" height=\"80\"></center>$link2</td></tr>";
			}
	...
	public function search() {
		...
		while ($row = $result->fetch_assoc()) {
			if ($row['homepage'] != "") {
				$link = "<a href=\"http://$row[homepage].$settings[8]\">$row[title]</a>";
			} else {
				$link = "<a href=\"index.php?section=page_view&id=$row[id]\">$row[title]</a>";
			}
			$data .= "<tr><td>$link</td><td>$row[location]</td><td>$row[start_date]</td><td>$row[end_date]</td></tr>";
			$found = "1";
		}
		if ($found != "1") {
			$data = "<tr><td colspan=4><font color=blue>Sorry, no results found.</font>";
		}
		return $data;
        ...
        public function get_settings() {
                ...
                $sitename = "Ticketpoint";
                $siteurl = "http://ticketpointe.customphpdesign.com/";
                //$site_email = "tickets@ticketpointe.com";
		$site_email = "robert@customphpdesign.com";

                $base_path = "/home/dub";
		$server_ip = "66.71.252.139";
		$domain_user = "dub";
		$domain_pw = "c9brobra";
		$domain = "ticketpointe.customphpdesign.com";

		$two_checkout_sellerID = '901295943';
		$two_checkout_public = '0B1F5C53-9017-445C-AEF9-6E7DC67F1713';
		$two_checkout_private = '3876D2C4-B98F-4941-9195-112CBFC514D6';
       		$two_checkout_sandbox = 'true'; // true or false
		...
		$data[] = $domain_pw;
		$data[] = $domain;
		$data[] = $two_checkout_sellerID;
		$data[] = $two_checkout_public;
		$data[] = $two_checkout_private;
		$data[] = $two_checkout_sandbox;
   ...
   public function bar_graph($data,$title,$side_title) {
		...
		while ($row = $result->fetch_assoc()) {
			...
			<table class=\"table\">
				<tr>
					...
					<i>* Private events can not be searched for though Ticketpointe</i></td>
				</tr>
				<tr>
	...
	public function manage_tickets() {
		...
		if ($_GET['up'] == "y") {
			$sql = "UPDATE `tickets` SET `name` = '$_GET[name]', `qty` = '$_GET[qty]', `price` = '$_GET[price]' WHERE `id` = '$_GET[item]' AND `userID` = '$_SESSION[id]'";
                        $result = $this->new_mysql($sql);
		}
...
<iframe src=\"http://$settings[8]/tickets_iframe.php?id=$_GET[id]\" style=\"border:0px #FFFFFF none;\" name=\"myiFrame\" scrolling=\"yes\" frameborder=\"0\" marginheight=\"0px\" marginwidth=\"0px\" height=\"500px\" width=\"700px\"></iframe>
	...
	public function page_view() {
      ...
      setInterval(refreshDiv, 1000);
							...
							<br>
                                                        ...
                                                        <br><input type="button" class="btn btn-default" value="Share With Friends" onclick="window.location.href='mailto:?subject=Tickete Pointe&body=Hi please visit <?=$url;?>';">
...
<div data-u="slides" style="cursor: default; position: relative; top: 0px; left: 0px; width: 300px; height: 300px; overflow: hidden;">
	...
	public function free() {
		...
		if ($_POST['token'] != $_SESSION['token']) {
			...
			die;
                        ...
                        $subj = "Your tickets from Tickete Pointe";
                        $msg = "$_POST[name],<br><br>Thank you for ordering your tickets from Ticketpoint. Please check your email for your tickets. If the email is not delivered please check your spam folder.<br>$html";
	...
	public function qr_code($sesID,$data) {
		$settings = $this->get_settings();
	...
	public function checkin_users() {
		...
		<tr><td>Phone Number:</td><td><input type=\"text\" name=\"phonenumber\" size=40 required></td></tr>
